...
\begin{table}[t]
    \centering
    \small
    \caption{Core notation used in the replay and optimizer derivations.}
    \label{tab:notation}
    \begin{tabular}{|>{\raggedright\arraybackslash}p{2.1cm}|>{\raggedright\arraybackslash}p{2.6cm}|>{\raggedright\arraybackslash}p{8.3cm}|}
        \hline
        Symbol & Role & Meaning in the current implementation \\
        \hline
        $\ell$ & leg index & One thermodynamic leg of the configured cycle. \\
        \hline
        $m$ & target state & Lambda state at which a replayed free energy or observable is evaluated. \\
        \hline
        $i_n$ & visited state & Lambda state visited by stored frame $n$ in the frozen reference trajectory. \\
        \hline
        $n$ & frame index & Stored frame index after the probe or production frame-selection rules. \\
        \hline
        $\theta$ & physical parameters & Trainable force-field parameters in their physical units. \\
        \hline
        $\phi$ & latent parameters & Optimizer coordinates mapped into physical parameters through $\theta = T(\phi)$. \\
        \hline
        $g_{\mathrm{ref},\ell}(i)$ & frozen Gibbs weights & Frozen AWH free-energy estimate plus the fixed target-state log weight for state $i$. \\
        \hline
        $\overline{w}^{\mathrm{rep}}_{\ell,n,m}$ & replay weights & Target-state weights used for endpoint free energies, endpoint gradients, and state-conditioned observables. \\
        \hline
        $\overline{w}^{\mathrm{ens}}_{\ell,n}$ & candidate-ensemble weights & Candidate-ensemble weights used for optimizer-side ESS and empirical Fisher estimates. \\
        \hline
    \end{tabular}
\end{table}
...
This pointwise normalizer is the same extended-ensemble denominator that appears in MBAR-style estimators \citep{bennett1976bar,shirts2008mbar}: at fixed $(\mathbf{x}, V)$ one sums over all states using reduced potentials and log state weights, then uses that denominator to normalize the contribution of any chosen target state. What is standard here is the denominator structure itself. What is specific to the present method is that the state weights are frozen AWH Gibbs weights, the data come from frozen-bias production, and the resulting replay estimators feed a later natural-gradient-like optimizer rather than a standalone free-energy postprocessor.
...
The implemented objective does not act on this raw residual directly. Instead, the cycle target is assigned a tolerance $s_{\mathrm{cyc}} > 0$ expressed in the same native units as the target before conversion to reduced units. By default the code uses a relative tolerance of $0.1\%$ of the target magnitude together with a small absolute floor. After converting the target and its tolerance into reduced units, the optimizer works with the normalized residual

\begin{equation}
    z_{\mathrm{cyc}}(\theta)
    =
    \frac{\mathcal{E}(\theta)}{s_{\mathrm{cyc}}}.
    \label{eq:normalized_cycle_residual}
\end{equation}

Let $H_\delta(z)$ denote the Huber loss with threshold $\delta$, and let $w_{\mathrm{cyc}}$ be the dimensionless priority weight attached to the cycle target. The implemented cycle contribution to the objective is then

\begin{equation}
    \mathcal{L}_{\mathrm{cyc}}(\theta)
    =
    w_{\mathrm{cyc}} H_\delta\!\left(z_{\mathrm{cyc}}(\theta)\right).
    \label{eq:cycle_huber_loss}
\end{equation}

Its gradient is

\begin{equation}
    \nabla_\theta \mathcal{L}_{\mathrm{cyc}}
    =
    w_{\mathrm{cyc}}
    \frac{d H_\delta}{dz}
    \frac{1}{s_{\mathrm{cyc}}}
    \nabla_\theta \Delta G_{\mathrm{cyc}}(\theta).
    \label{eq:lossgradtheta}
\end{equation}

The same normalization logic is later applied to observable targets, so heterogeneous quantities such as free energies and densities can be mixed without one raw unit system dominating the summed objective.
...
Each target is also assigned a tolerance $s_r > 0$ in its own native units. In the implemented code, $s_r$ defaults to $0.1\%$ of the target magnitude, again with a small absolute floor for near-zero targets, unless an explicit tolerance is supplied in the configuration. The optimizer therefore uses the normalized residual

\begin{equation}
    z_r(\theta)
    =
    \frac{\mathcal{E}_r(\theta)}{s_r},
    \label{eq:obs_normalized_residual}
\end{equation}

and the weighted Huber contribution

\begin{equation}
    \mathcal{L}_r(\theta)
    =
    w_r H_\delta\!\left(z_r(\theta)\right),
    \label{eq:obs_target_loss}
\end{equation}

where $w_r$ is a dimensionless priority multiplier. The corresponding gradient is

\begin{equation}
    \nabla_\theta \mathcal{L}_r(\theta)
    =
    w_r
    \frac{d H_\delta}{dz_r}
    \frac{1}{s_r}
...
This normalization also defines the default success criterion. A target is considered within specification whenever $|\mathcal{E}_r(\theta)| \le s_r$, that is, whenever its raw prediction error lies within the configured tolerance band. The optimizer therefore treats the $0.1\%$ band not only as a scaling convention for mixed-unit losses, but also as the default notion of per-target success.
...
The end-to-end control flow is summarized once, in implementation Section~\ref{sec:authoritative_control_flow}; the role of the present theory section is only to justify why the Fisher/KL pair is the right local geometry for that control flow. This is why a KL target, rather than a raw Euclidean step length, is the natural quantity to bound. A step that is small in Euclidean coordinates can still be large in distribution space if it distorts the ensemble strongly, while a step that is large in a weakly constrained coordinate may still be statistically safe. The Fisher metric captures that distinction directly.
...
The base KL target $\delta_{\mathrm{KL}}$ also determines the minimum acceptable effective sample size checked on each line-search step. A parameter change with predicted KL divergence equal to $\delta_{\mathrm{KL}}$ reduces the Kish ESS of the reference ensemble by a factor of approximately $\exp(-2\delta_{\mathrm{KL}})$. A short local justification is useful here. Let $r(\phi)$ denote the candidate-to-reference importance ratio, normalized so that $\mathbb{E}_{\mathrm{ref}}[r]=1$. Under the same local Gaussian-cumulant approximation that underlies the quadratic KL model, $\log r$ has mean approximately $-\delta_{\mathrm{KL}}$ and variance approximately $2\delta_{\mathrm{KL}}$. This implies

\begin{equation}
    \mathrm{CV}^2(r)
    \approx
    \exp\!\left(2\delta_{\mathrm{KL}}\right) - 1,
\end{equation}
%
so the Kish identity $\mathrm{ESS} \approx M_\ell /(1+\mathrm{CV}^2(r))$ gives the local rule of thumb $\mathrm{ESS} \approx M_\ell \exp(-2\delta_{\mathrm{KL}})$. The implementation uses that relation as an approximate support-calibration heuristic rather than as an exact theorem. The per-leg ESS threshold is therefore set to
...
\end{equation}
%
where $s_{\mathrm{ESS}} \in (0,1]$ is a configurable safety margin absorbing higher-order departures from the local KL picture, finite-sample noise, and temporal correlation between stored frames. In the current implementation, this ESS threshold is computed once from the base KL target and is not rescaled by the later confidence factor. The ESS is evaluated at the \emph{proposed} parameters on each halving step, so the line search rejects any proposal that would leave the reference ensemble with insufficient support for the candidate ensemble, independently of whether the residual has improved.

The target $\delta_{\mathrm{KL}}$ is itself moderated by a confidence factor $s_{\mathrm{conf}} \in [s_{\min}, 1]$. In the implementation, this factor is obtained by comparing optimization-relevant quantities estimated from the first and second halves of the production data, such as target predictions and their gradients. Large split-half disagreement is interpreted as a noise proxy rather than as a proof that the current replay estimates are correct or incorrect, in which case $s_{\mathrm{conf}}$ is reduced. This shrinks the trust region and makes the update more conservative. This confidence moderation is therefore an additional robustness heuristic layered on top of the KL trust region, not a separate geometric identity. More formal uncertainty estimates, such as block/bootstrap or replica-aware diagnostics, are natural future replacements for this heuristic but are not yet part of the current code.

The empirical Fisher matrix is therefore the operator that turns the Euclidean loss gradient into a natural-gradient-like base step, after which the optimizer applies the stabilization steps described above. The implementation details are summarized in Section~\ref{sec:authoritative_control_flow}; this theory section only fixes the geometric interpretation of those steps, not the practical readiness or line-search heuristics layered on top of them. This is why a KL target, rather than a raw Euclidean step length, is the natural quantity to bound. A step that is small in Euclidean coordinates can still be large in distribution space if it distorts the ensemble strongly, while a step that is large in a weakly constrained coordinate may still be statistically safe. The Fisher metric captures that distinction directly. In that sense, the replay machinery determines which direction lowers the objective, whereas the empirical Fisher determines how far the optimizer can move in that direction before the induced ensemble changes too much.
