...
Each target is also assigned a tolerance $s_r > 0$ in its own native units. In the implemented code, $s_r$ defaults to $5\%$ of the target magnitude, again with a small absolute floor for near-zero targets, unless an explicit tolerance is supplied in the configuration. The optimizer therefore uses the normalized residual
