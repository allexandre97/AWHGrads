...
The base step is formed by regularizing the Fisher matrix with a reference-model penalty, applying diagonal Jacobi preconditioning to account for parameter scales, and truncating the eigenvectors of the resulting correlation-like matrix to ignore statistically "slack" directions. The step size is then determined by scaling the resulting natural-gradient direction to satisfy a Kullback-Leibler (KL) divergence target in distribution space, followed by an infinity-norm cap on the latent update to ensure stability. The line search then proposes
...
The residual test is not a simple monotonic decrease. The current implementation accepts a proposal only if the new residual lies inside a tolerance-adjusted window based on the current residual magnitude, and the required improvement is made stricter when the production split-half confidence is poor.
...
\begin{itemize}
    ...
    \item the optimization can jointly train Lennard-Jones parameters and partial charges. When charge training is enabled, the code uses a constrained charge-equilibration algorithm (predicting electronegativity and hardness per atom class) to solve for partial charges under exact molecular net-charge constraints before evaluating the force-field energy,
